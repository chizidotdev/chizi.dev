...
<div class="flex flex-col gap-10">
  ...
  <section class="space-y-3 border-b">
    ...
    <div class="flex flex-col gap-3 py-6">
      <div class="flex items-center justify-between">
        <Text label>Total in NGN:</Text>
        <Text variant="h2">{totalAmount}</Text>
      </div>
      <div class="flex items-center justify-between">
        <Text label>Amount Due:</Text>
        <Text variant="h2">{amountDue}</Text>
      </div>
      <a
        href="{data.client}/receipt"
        class={cn(buttonStyles({ variant: "link" }), "mt-4 w-min self-end")}
      >
        View Receipts
      </a>
    </div>
  ...
  <section
    class="sticky bottom-0 -mx-2 flex items-center justify-between gap-3 border-t bg-background px-2 py-6"
  >
    <a
